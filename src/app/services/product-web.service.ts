...
export class ProductWebService {
  ...
  post(url: string, data: Product): Observable<any>{
  ...
  delete(url: string, data: Product): Observable<any> {
    return this.httpClient.delete(this.serverUrl + url, { params: { id: data.productId + "" } });
  }
}
